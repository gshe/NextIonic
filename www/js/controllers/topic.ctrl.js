...
.controller('TopicCtrl', function($rootScope, $scope, $stateParams, $ionicLoading, $ionicActionSheet, $ionicScrollDelegate, $log, Topic, Topics, User, ENV) {
  $log.debug('topic ctrl', $stateParams);
  var id = $stateParams.id;
  var topic = Topics.getById(id);
  $scope.topic = topic;
  var currentUser = User.getCurrentUser();
  $scope.loginname = currentUser.loginname;
  ...
  $scope.shareTopic = function() {
    if (window.plugins && window.plugins.socialsharing) {
      window.plugins.socialsharing.available(function(isAvailable) {
        if (isAvailable) {
          window.plugins.socialsharing.share(topic.title, 'Ionic社区话题分享', 'http://ww2.sinaimg.cn/large/658e3191gw1eyw4vrjbkaj2040040mx4.jpg', ENV.domain + '/topic/' + topic.id);
        } else {
          alert('分享失败');
        }
      });
